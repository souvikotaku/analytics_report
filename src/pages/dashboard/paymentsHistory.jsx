        ...
        <Card>
          ...
          <CardBody className="overflow-x-scroll px-0 pt-0 pb-2">
            <table className="w-full min-w-[640px] table-auto">
              <thead>
                <tr>
                  {["Customer", "Payment Date", "Status", "Amount"].map((el) => (
                    <th
                      key={el}
              ...
              <tbody>
                {paymentTableData.map(
                  ({name,date,status,amount }, key) => {
                    const className = `py-3 px-5 ${
                      key === paymentTableData.length - 1
                        ? ""
                    ...
                    return (
                      <tr key={name}>
                        ...
                        <td className={className}>
                          <Chip
                            variant="gradient"
                            color={status === "Successful" ? "green" : status === "Pending" ? "yellow" : "red"}
                            value={status}
